...
const description = 'Construct a markdown from an array of images and captions.';
...
export const images_to_markdown_component = createComponent(group_id, id, title, category, description, summary, links, inputs, outputs, controls, parsePayload);
...
async function parsePayload(payload, ctx) 
{
  ...
  if ((!images_cdns || images_cdns.length == 0) && (!captions_object || captions.length == 0)) { throw new Error(`No images or captions specified`); }
  ...
  if (!entry_name || entry_name == "") {info += `No Entry Name specified\n`;}
